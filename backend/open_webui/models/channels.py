...
class ChannelTable:
    ...
    def delete_channel_by_id(self, id: str):
        with get_db() as db:
            # Clean up associated messages and their reactions
            from open_webui.models.messages import Message, MessageReaction

            message_ids = [
                m.id for m in db.query(Message.id).filter_by(channel_id=id).all()
            ]
            if message_ids:
                db.query(MessageReaction).filter(
                    MessageReaction.message_id.in_(message_ids)
                ).delete(synchronize_session=False)
            db.query(Message).filter_by(channel_id=id).delete()
