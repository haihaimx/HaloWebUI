...
class MessageTable:
    ...
    def delete_message_by_id(self, id: str) -> bool:
        with get_db() as db:
            # Delete reply messages first
            self.delete_replies_by_id(id)
    ...
    def delete_messages_by_user_id(self, user_id: str) -> bool:
        with get_db() as db:
            try:
                # Delete reactions for all messages by this user
                user_message_ids = [
                    m.id for m in db.query(Message.id).filter_by(user_id=user_id).all()
                ]
                if user_message_ids:
                    db.query(MessageReaction).filter(
                        MessageReaction.message_id.in_(user_message_ids)
