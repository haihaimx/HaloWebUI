	...
	<Select.Trigger class="relative w-full" aria-label={placeholder}>
		{#if selectedItem}
			<OverflowTooltip
				...
				textClassName="inline-flex h-input px-0.5 w-full outline-hidden bg-transparent truncate text-lg font-semibold focus:outline-hidden"
			>
				{selectedItem.label}
			</OverflowTooltip>
		{:else}
			<span
				class="inline-flex h-input px-0.5 w-full pr-7 outline-hidden bg-transparent truncate text-lg font-semibold text-gray-400 focus:outline-hidden"
			>
				{placeholder}
			</span>
		{/if}
