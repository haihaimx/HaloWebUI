<script lang="ts">
	...
	const transcribeHandler = async (audioBlob) => {
		...
		const sttLanguage = $settings?.audio?.stt?.language ?? '';

		const res = await transcribeAudio(localStorage.token, file, sttLanguage || undefined).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			}
       ...
       font-medium flex-1 mx-auto text-center"
		...
		<div class="flex items-center">
			...
			{:else}
				<button
					type="button"
					class="p-[7px] bg-indigo-500 text-white dark:bg-indigo-500 dark:text-blue-950 rounded-full"
					on:click={async () => {
