<script>
	...
	$: folderIcon = folders[folderId]?.meta?.icon || '';

	const iconUpdateHandler = async (icon) => {
		const res = await updateFolderIconById(localStorage.token, folderId, icon || null).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			}
...
{#if showIconPicker}
	...
	>
		<div
			class="absolute left-16 top-1/3 bg-white dark:bg-gray-850 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-3 w-56"
		...
		>
			...
			<div class="flex flex-wrap gap-1.5 mb-2">
				{#each ['📁', '📂', '📌', '⭐', '❤️', '🔥', '💡', '🎯', '📝', '💻', '🎨', '🔧', '📚', '🏠', '🚀', '🎵'] as emojiOption}
					<button
						class="w-7 h-7 flex items-center justify-center text-base rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition {folderIcon ===
						emojiOption
							? 'bg-gray-200 dark:bg-gray-600 ring-1 ring-blue-400'
							: ''}"
						on:click={() => iconUpdateHandler(emojiOption)}
					>
						{emojiOption}
					</button>
				{/each}
			...
			{#if folderIcon}
				<button
					class="w-full text-xs text-center py-1 text-gray-500 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-400 transition"
					on:click={() => iconUpdateHandler('')}
				>
					{$i18n.t('Remove Icon')}
				</button>
			{/if}
