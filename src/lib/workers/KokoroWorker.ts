export class KokoroWorker {
	...
	constructor(options: { dtype?: string } | string = 'fp32') {
		this.dtype = typeof options === 'string' ? options : (options?.dtype ?? 'fp32');
	}
	...
	public async init() {
		...
		const { default: WorkerInstance } = await import('$lib/workers/kokoro.worker?worker');
